...
where I: Iterator<Item = (&'a u32, &'a Vec<char>)>, {
    old.fold(
        HashMap::new(),
        |mut acc, (key, value)| {
            for c in value {
                acc.entry(c.to_ascii_lowercase()).or_insert(*key);
            }
            acc
        }
    )
...
pub fn transform(last: &HashMap<u32, Vec<char>>) -> HashMap<char, u32> {
    transform_iter(last.iter())
}
